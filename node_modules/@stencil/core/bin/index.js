...
function getNodeSys(distRootDir, logger) {
    ...
    var packageJsonData;
    try {
        packageJsonData = require(path$$1.join(distRootDir, 'package.json'));
    }
    catch (e) {
    ...
    var sys = {
        ...
        getClientCoreFile: function (opts) {
            var filePath = path$$1.join(distRootDir, 'client', opts.staticName);
            return new Promise(function (resolve, reject) {
        ...
        loadConfigFile: function (configPath) {
            ...
            if (!config.rootDir) {
                config.rootDir = path$$1.dirname(configPath);
            }
            return config;
        },
        isGlob: function (str) {
...
function loadConfigFile(process, configPath, logger) {
    ...
    try {
        ...
        if (fileStat.isDirectory()) {
            ...
            configPath = path$$1.join(configPath, 'stencil.config.js');
        }
...
function run(process, minNodeVersion, logger) {
    ...
    var configPath = getConfigFilePath(process, argv.config);
    ...
    switch (task) {
        case 'build':
            ...
            break;
        case 'docs':
            stencil.docs(config).catch(function (err) {
                config.logger.error(err);
            });
            break;
        default:
