...
function renderUpdate(plt, elm, isInitialLoad) {
    ...
    try {
        render$1(plt, elm, plt.getComponentMeta(elm), !isInitialLoad);
...
function mockLogger() {
    var logger = {
        ...
        green: function (msg) { return msg; },
        yellow: function (msg) { return msg; },
        blue: function (msg) { return msg; },
        magenta: function (msg) { return msg; },
        cyan: function (msg) { return msg; },
        gray: function (msg) { return msg; },
        bold: function (msg) { return msg; },
        dim: function (msg) { return msg; }
    };
    return logger;
}
...
function getPropDecoratorMeta(tsFilePath, diagnostics, classNode, sourceFile, transformContext) {
    ...
    return decoratedMembers
        .filter(function (prop) { return (prop.decorators.some(function (decorator) { return decorator.getFullText().indexOf('Prop(') !== -1; })); })
        .reduce(function (allMembers, prop) {
        var memberData = {};
        var propDecorator = prop.decorators.find(function (decorator) { return (decorator.getFullText().indexOf('Prop(') !== -1); });
        var suppliedOptions = propDecorator.expression.arguments
            .map(function (arg) {
            try {
                var fnStr = "return " + arg.getText() + ";";
        ...
        var propOptions = suppliedOptions[0];
        var attribName = prop.name.text;
        if (propOptions && typeof propOptions.connect === 'string') {
            ...
            var attribType = void 0;
            // If the @Prop() attribute does not have a defined type then infer it
            if (!prop.type) {
                var attribTypeText = inferPropType(prop.initializer);
                if (!attribTypeText) {
                    attribTypeText = 'any';
                    diagnostics.push({
                        level: 'warn',
...
function getAttributeTypeInfo(type, sourceFile, transformContext) {
    ...
    var typeReferences = getAllTypeReferences(type, transformContext)
        .reduce(function (allReferences, rt) {
        ...
        switch (node.kind) {
            case ts.SyntaxKind.TypeReference:
                ...
                if (node.typeArguments) {
                    node.typeArguments
                        .filter(function (ta) { return ts.isTypeReferenceNode(ta); })
                        .forEach(function (tr) { return referencedTypes.push(tr.typeName.getText().trim()); });
                }
            default:
                return ts.visitEachChild(node, function (node) {
                    return visit(node);
...
function getTypeReferenceLocation(typeName, sourceFile) {
    ...
    var importTypeDeclaration = sourceFile.statements.find(function (st) {
        var statement = ts.isImportDeclaration(st) &&
            ...
            st.importClause.namedBindings.elements.find(function (nbe) { return nbe.name.getText() === typeName; });
        if (!statement) {
            return false;
        }
        return true;
    ...
    var isExported = sourceFile.statements.some(function (st) {
        // Is the interface defined in the file and exported
        var isInterfaceDeclarationExported = ((ts.isInterfaceDeclaration(st) &&
            st.name.getText() === typeName) &&
            Array.isArray(st.modifiers) &&
            st.modifiers.some(function (mod) { return mod.kind === ts.SyntaxKind.ExportKeyword; }));
        // Is the interface exported through a named export
        var isTypeInExportDeclaration = ts.isExportDeclaration(st) &&
            ts.isNamedExports(st.exportClause) &&
            st.exportClause.elements.some(function (nee) { return nee.name.getText() === typeName; });
...
function componentModuleFileClass(config, fileMeta, diagnostics) {
    return function (transformContext) {
        ...
        return function (tsSourceFile) {
            return visit(fileMeta, tsSourceFile, tsSourceFile);
        };
    };
}
...
function visitClass(config, moduleFile, diagnostics, classNode, sourceFile, transformContext) {
    ...
    moduleFile.cmpMeta = __assign$2({}, cmpMeta, { componentClass: classNode.name.getText().trim(), membersMeta: __assign$2({}, getElementDecoratorMeta(classNode), getMethodDecoratorMeta(classNode), getStateDecoratorMeta(classNode), getPropDecoratorMeta(moduleFile.tsFilePath, diagnostics, classNode, sourceFile, transformContext)), eventsMeta: getEventDecoratorMeta(moduleFile.tsFilePath, diagnostics, classNode), listenersMeta: getListenDecoratorMeta(moduleFile.tsFilePath, diagnostics, classNode) }, getPropChangeDecoratorMeta(classNode));
